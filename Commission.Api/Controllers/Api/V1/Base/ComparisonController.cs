...
namespace Commission.Api.Controllers.Api.V1.Base
{
    ...
    public class ComparisonController : ControllerBase
    {
        ...
        private ResponseModel UpdateIsDelete(CommonEnum.IsDeleted isDeleted, string ids)
        {
            ...
            {
                var response = ResponseModelFactory.CreateInstance;
                DataTable dtCheck = _dbContext.Database.SqlQuery(string.Format(@"SELECT 1 FROM dbo.vBillRecord WHERE FComparisonID IN ({0})", ids));
                if (dtCheck != null && dtCheck.Rows.Count > 0)
                {
                    response.SetFailed("发现提成比例对照档案已经存在记录,无法删除!");
                    return response;
                }
                else
                {
